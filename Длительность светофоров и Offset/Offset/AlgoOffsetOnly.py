...
def plot_real_traffic_wave(algo_name, time_limit=600, v_kmh=50):
    ...
    ax.set_title(f'Пространственно-временная диаграмма («Зеленая волна») — Алгоритм: {algo_name}', fontsize=16)
    ax.set_xlabel('Время (секунды)', fontsize=12)
    ax.set_ylabel('Расстояние вдоль коридора (метры)', fontsize=12, labelpad=50)
    ax.set_xlim(10, time_limit)
    ax.set_ylim(-80, max(distances.values()) + 100)
    ax.grid(True, linestyle='--', alpha=0.5)

    # Добавление понятной легенды
    legend_elements = [
        Patch(facecolor='#2ecc71', label='Основная целевая зеленая фаза'),
        Patch(facecolor='#e74c3c', label='Основная целевая красная фаза'),
        Patch(facecolor='#f1c40f', label='Переходные фазы (желтый и др.)'),
        plt.Line2D([0], [0], color='blue', lw=2, label=f'Траектория ТС ({v_kmh} км/ч)')
    ]
    ax.legend(handles=legend_elements, loc='upper right')

    plt.tight_layout()
    plt.savefig('Offset_OffsetOnly.png', bbox_inches='tight')



# Запуск
plot_real_traffic_wave("OffsetOnly")